...
class CiscoClientNDFC(CiscoClientController):
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    "NDFC authentication failed with status: %s", response.status_code
                )
    ...
    def _detect_msd_fabric_from_endpoints(
        ...
        try:
            ...
            if msd_root and self.fabric_name == msd_root:
                ...
                logger.info(
                    "Fabric %s is MSD root with members: %s",
                    self.fabric_name,
                    member_fabrics,
                )
            else:
                ...
                self.is_msd_fabric = False
                logger.info(
                    "Fabric %s is not MSD root - treating as standalone fabric",
                    self.fabric_name,
                )
                if msd_root:
    ...
    def _extract_fabric_id_from_endpoints(
        ...
        if not fabric_config_endpoint:
            logger.info(
                "No Fabric_Configuration endpoint found - fabric ID will not be available"
            )
            return
            ...
            if self.is_msd_fabric:
                ...
                for fabric_data in result[endpoint_name]:
                    if fabric_data.get("fabric") == self.fabric_name:
                        ...
                        if isinstance(data, dict) and "id" in data:
                            ...
                            logger.info(
                                "Extracted fabric ID %s for MSD fabric %s",
                                self.fabric_id,
                                self.fabric_name,
                            )
                            break
                        elif (
                            isinstance(data, list) and len(data) > 0 and "id" in data[0]
                        ):
                ...
                if result[endpoint_name]:
                    ...
                    if isinstance(data, dict) and "id" in data:
                        self.fabric_id = data["id"]
                        logger.info(
                            "Extracted fabric ID %s for fabric %s",
                            self.fabric_id,
                            self.fabric_name,
                        )
    ...
    def _process_msd_endpoint(
        ...
        if self.fabric_id and "{{fabricID}}" in endpoint_url:
            ...
            logger.debug(
                "Replaced {{fabricID}} with %s in MSD endpoint URL", self.fabric_id
            )
    ...
    def _process_endpoint_for_msd_fabrics(
        ...
        for fabric_name in self.msd_topology.get("all_fabrics", []):
            logger.info(
                "Processing endpoint %s for MSD fabric: %s", endpoint_name, fabric_name
            )

            # Skip Discovered_Switches for MSD root fabric (fabricType: "MSD")
            if endpoint_name == "Discovered_Switches" and self._is_msd_root_fabric(
                fabric_name
            ):
                logger.info(
                    "Skipping Discovered_Switches for MSD root fabric: %s", fabric_name
                )
                continue
            ...
            ] and not self._is_msd_root_fabric(fabric_name):
                logger.info(
                    "Skipping %s for MSD child fabric: %s (collect only from MSD root)",
                    endpoint_name,
                    fabric_name,
                )
                continue
            ...
            try:
                ...
                logger.error(
                    "Error processing endpoint %s for fabric %s: %s",
                    endpoint_name,
                    fabric_name,
                    str(e),
                )
    ...
    def _process_endpoint_single_site(
        ...
        if self.fabric_id and "{{fabricID}}" in endpoint_url:
            endpoint_url = endpoint_url.replace("{{fabricID}}", str(self.fabric_id))
            logger.debug(
                "Replaced {{fabricID}} with %s in endpoint URL for fabric %s",
                self.fabric_id,
                self.fabric_name,
            )
    ...
    def _update_fabric_id_for_current_fabric(self, result: dict[str, Any]) -> None:
        ...
        if "Fabric_Configuration" not in result:
            logger.warning(
                "No Fabric_Configuration data available to extract fabric ID for %s",
                self.fabric_name,
            )
            return
        ...
        for fabric_data in result["Fabric_Configuration"]:
            ...
            if fabric_from_data == self.fabric_name:
                ...
                if isinstance(data, dict) and "id" in data:
                    self.fabric_id = data["id"]
                    logger.debug(
                        "Updated fabric ID to %s for fabric %s",
                        self.fabric_id,
                        self.fabric_name,
                    )
                    return
        ...
        logger.warning(
            "Could not find fabric ID for fabric %s in Fabric_Configuration data",
            self.fabric_name,
        )
    ...
    def _process_policies_endpoint_with_filtering(
        ...
        if self.fabric_id and "{{fabricID}}" in endpoint_url:
            endpoint_url = endpoint_url.replace("{{fabricID}}", str(self.fabric_id))
            logger.debug(
                "Replaced {{fabricID}} with %s in Policies endpoint URL", self.fabric_id
            )
        ...
        try:
            ...
            if all_policies_data is not None:
                ...
                if not serial_numbers:
                    logger.warning(
                        "No serial numbers found in Discovered_Switches data"
                    )
                    endpoint_dict[endpoint_name].append(
                        {
                            "data": {},
                ...
                filtered_policies = self._filter_autogenerated_policies(
                    filtered_policies
                )
    ...
    def _filter_specyfic_template_policies(
        ...
        logger.debug(
            "Template filtering: excluding templates %s from %d policies",
            template_names,
            len(policies_list),
        )
    ...
    def _fix_escaped_json_in_data(self, data: Any) -> None:
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if key in ["vrfTemplateConfig", "networkTemplateConfig"] and isinstance(
                    value, str
                ):
                    try:
                        ...
                        logger.warning(
                            "Could not parse %s field as JSON: %s", key, str(e)
                        )
                    ...
                    if stripped_value and stripped_value[0] in "{[":
                        try:
                            ...
                            logger.warning(
                                "Could not parse dhcpServers field as JSON: %s", str(e)
                            )
                else:
    ...
    def _process_switch_interfaces(
        ...
        for config_index, config_entry in enumerate(endpoint_dict[parent_name]):
            ...
            if self.is_msd_fabric and entry_fabric and entry_fabric != self.fabric_name:
                logger.debug(
                    "Skipping Discovered_Switches entry for fabric %s (currently processing %s)",
                    entry_fabric,
                    self.fabric_name,
                )
                continue
            ...
            logger.debug(
                "Processing Discovered_Switches entry %d for fabric %s",
                config_index,
                entry_fabric,
            )
            ...
            for _switch_index, switch in enumerate(switches_data):
                ...
                if (
                    switch_fabric_name
                    and current_fabric
                    and switch_fabric_name != current_fabric
                ):
                    logger.debug(
                ...
                for child_endpoint in children_endpoints:
                    ...
                    logger.debug(
                        "Fetching %s interfaces from: %s", child_name, child_url
                    )
                    ...
                    try:
                        ...
                        if interface_data is not None:
                            ...
                            processed_interfaces = self._process_interface_data(
                                interface_data
                            )
                            ...
                            if processed_interfaces:
                                ...
                                if "children" in child_endpoint and isinstance(
                                    child_endpoint["children"], list
                                ):
                                    logger.debug(
                            ...
                            logger.info(
                                "Added %d %s interfaces for switch: %s",
                                len(processed_interfaces)
    ...
    def _process_vrf_attachments(
        ...
        for _config_index, config_entry in enumerate(endpoint_dict[parent_name]):
            if not config_entry.get("data"):
                continue
            ...
            for _vrf_index, vrf in enumerate(vrf_data):
                vrf_status = vrf.get("vrfStatus")
                vrf_name = vrf.get("vrfName")

                if vrf_status == "DEPLOYED" and vrf_name:
                    logger.info(
                        "Processing VRF_Attachments for deployed vrf: %s", vrf_name
                    )
                    ...
                    try:
                        ...
                        if attachment_data is not None:
                            logger.debug(
                                "Successfully retrieved vrf attachments for: %s",
                                vrf_name,
                            )

                            # Process the attachment data
                            processed_attachments = self._process_attachment_data(
                                attachment_data
                            )
                            ...
                            logger.info(
                                "Added %d vrf attachments for vrf: %s",
                                len(processed_attachments)
                                if isinstance(processed_attachments, list)
                                else 1,
                                vrf_name,
                            )
                        else:
                            logger.warning(
                                "Failed to fetch vrf attachments for: %s", vrf_name
                            )
                            vrf["vrf_attach_group"] = []

                    except Exception as e:
                        logger.error(
                            "Error fetching vrf attachments for %s: %s",
                            vrf_name,
                            str(e),
                        )
                        vrf["vrf_attach_group"] = []
        ...
        for _config_index, config_entry in enumerate(endpoint_dict[parent_name]):
            ...
            for _network_index, network in enumerate(networks_data):
                ...
                if network_status == "DEPLOYED" and network_name:
                    ...
                    logger.debug(
                        "Fetching network attachments from: %s", attachment_url
                    )
                    ...
                    try:
                        ...
                        if attachment_data is not None:
                            logger.debug(
                                "Successfully retrieved network attachments for: %s",
                                network_name,
                            )
                            ...
                            logger.info(
                                "Added %d network attachments for network: %s",
                                len(processed_attachments)
                                if isinstance(processed_attachments, list)
                                else 1,
                                network_name,
                            ...
                            logger.warning(
                                "Failed to fetch network attachments for: %s",
                                network_name,
                            )
                        ...
                        logger.error(
                            "Error fetching network attachments for %s: %s",
                            network_name,
                            str(e),
                        )
                        network["network_attach_group"] = []
                    ...
                    if network_status != "DEPLOYED":
                        logger.debug(
                            "Skipping network %s with status: %s",
                            network_name or "unnamed",
                            network_status,
                        )
                    else:
    ...
    def _process_vpc_pairs_children(
        ...
        for vpc_pair_data in all_vpc_pairs:
            ...
            logger.debug(
                "Processing %s children for peerOneId=%s", parent_name, peer_one_id
            )
            ...
            for child_endpoint in children_endpoints:
                ...
                try:
                    ...
                    processed_count += 1
                    logger.debug(
                        "Successfully processed and saved child endpoint %s to %s entry for peerOneId=%s",
                        child_name,
                        parent_name,
                        peer_one_id,
                    )
                    ...
                    logger.error(
                        "Error processing child endpoint %s for peerOneId=%s: %s",
                        child_name,
                        peer_one_id,
                        str(e),
                    )
        ...
        for interface_entry in (
            ...
            if parent_name in self.PORT_CHANNEL_INTERFACE_TYPES:
                ...
                if not vpc_entity_id:
                    logger.debug(
                        "No vpcEntityId found in %s interface, skipping nested children",
                        parent_name,
                    )
                    continue
                ...
                for child_endpoint in children_endpoints:
                    ...
                    logger.debug(
                        "Fetching nested child endpoint: %s -> %s",
                        child_name,
                        child_url,
                    )
            ...
            # For serial-based interfaces (LoopbackInterfaces, etc.), look for serialNo and ifName
                ...
                if not serial_number or not if_name:
                    logger.debug(
                        "No serialNo or ifName found in %s interface, skipping nested children",
                        parent_name,
                    )
                    continue
    ...
    def _parse_vpc_entity_id(self, vpc_entity_id: str) -> tuple[str | None, str | None]:
        ...
        if len(parts) != 3:
            logger.warning(
                "vpcEntityId does not have expected format 'serial1~serial2~vpcX': %s",
                vpc_entity_id,
            )
            return None, None
        ...
        logger.debug(
            "Parsed vpcEntityId '%s' -> vpcPair='%s', vPC_name='%s'",
            vpc_entity_id,
            vpc_pair,
            vpc_name,
        )
